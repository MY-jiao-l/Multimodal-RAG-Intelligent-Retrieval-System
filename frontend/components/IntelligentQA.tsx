...
import { Search, Loader2, MessageSquare, Upload, FileImage } from "lucide-react";
...
export function IntelligentQA() {
  ...
  const handleAsk = async (e: React.FormEvent) => {
    ...
    if (uploadedFile && !isUploading && !uploadedFileId) {
      await handleUpload();
    }

    await ask(question, undefined, 3, uploadedFileId || undefined);
  };
          ...
          {!isLoading && answer && queryType ? (
            ...
            <div className="space-y-6">
              ...
              <form onSubmit={handleAsk}>
                <div className="relative">
                  <Input
                    ...
                    className="h-14 pl-12 pr-32 text-[15px] backdrop-blur-xl bg-transparent border-border/60 rounded-2xl shadow-lg focus-visible:ring-2 focus-visible:ring-primary/30 text-foreground font-semibold placeholder:text-muted-foreground placeholder:font-normal"
                  />
              ...
              <div className="flex flex-wrap gap-2 justify-center">
                <span className="text-[12px] text-muted-foreground">试试这些问题：</span>
                {[
                  "这张图中有几个卧室？",
                  "总面积是多少？",
                  "建筑的长度和宽度是多少？",
                  ...
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    className="h-7 text-[11px] border-border/60 text-foreground hover:border-primary/40 hover:bg-primary/10 hover:text-primary"
              ...
              <div className="text-center py-8">
                ...
                <h3 className="text-[16px] font-medium text-foreground/90 mb-2">
                  支持文本+图像/CAD 混合检索
                </h3>
        ...
        <aside className="w-[420px] backdrop-blur-xl bg-card/60 border-l border-border/50 overflow-y-auto shadow-2xl shadow-black/20">
          <div className="sticky top-0 backdrop-blur-xl bg-gradient-to-r from-card/90 to-card/80 border-b border-border/50 px-6 py-4 z-10">
            <div className="flex items-center justify-between">
              <h3 className="text-[16px] font-semibold text-foreground/90 flex items-center gap-2">
                来源文档
              </h3>
            </div>
          </div>
              ...
              >
                <div className="flex items-start gap-2 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h5 className="text-[13px] font-medium text-foreground/90 mb-2">
                      {source.file_name}
                    </h5>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <Badge
                        variant="default"
                        className="text-[10px] px-2 py-0.5 bg-gradient-to-r from-emerald-500/20 to-cyan-500/20 text-emerald-600 border-emerald-500/30"
                      >
